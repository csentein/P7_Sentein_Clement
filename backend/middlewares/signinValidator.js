...
module.exports = (req, res, next) => {
    ...
    if (req.body.firstName != null && /[A-zÀ-ú]/.test(req.body.firstName))
        isFirstName = validator.matches(String(req.body.firstName), regex); // Récupération du firstName envoyé par le frontEnd et attribution à la variable "isFirstName" (vérification de la conformité à l'aide de la regex)
    if (req.body.lastName != null && /[A-zÀ-ú]/.test(req.body.lastName))
        isLastName = validator.matches(String(req.body.lastName), regex);
    if (req.body.email != null && /[A-zÀ-ú]/.test(req.body.email))
        isEmail = validator.isEmail(String(req.body.email)); // "validator.isEmail" permet de vérifier si "req.body.email" est bien au format email (càd : un @, un point et pas de caractères spéciaux/maj)

    if (isFirstName && isLastName && isEmail) { // Si le schéma du mot de passe est valide, exécution du code suivant
        next()
    } else {
